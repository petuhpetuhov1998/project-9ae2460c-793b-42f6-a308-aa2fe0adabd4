  ...
  return (
    <>
      ...
      >
        ...
        {/* Gradient overlay */}
        <span className="absolute inset-0 rounded-full bg-gradient-to-br from-white/20 via-transparent to-transparent opacity-60" />
        
        {/* Inner glow */}
        <span className="absolute inset-[2px] rounded-full bg-gradient-to-b from-white/10 to-transparent" />

        {/* Content */}
        <span className="relative z-10 px-8 py-4 font-monly font-bold text-lg md:text-xl text-white group-hover:text-white transition-colors">
          {children}
        </span>

        {/* Shine effect */}
        <span className="absolute top-0 left-1/4 w-1/2 h-[1px] bg-gradient-to-r from-transparent via-white/60 to-transparent" />
      </Comp>
      <GlassFilter />
    </>
  )
    ...
    >
      ...
      >
        ...
        {isHovered && !isPressed && !isTouchDevice && (
          <span className="absolute inset-0 rounded-md ring-2 ring-white/20 pointer-events-none" />
        )}
      </button>
    </span>
  );
